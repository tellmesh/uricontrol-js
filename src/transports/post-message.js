export class PostMessageBridge {
  ...
  attachRuntime(runtime) {
    globalThis.addEventListener?.('message', async (event) => {
      ...
      event.source?.postMessage({ protocol: 'uricontrol', type: 'result', id: msg.id, result }, event.origin || '*');
    });
  }

  call(uri, payload = {}, context = {}) {
    const id = `pm_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    this.targetWindow.postMessage({ protocol: 'uricontrol', type: 'call', id, uri, payload, context }, this.targetOrigin);
  ...
  #onMessage(event) {
    const msg = event.data;
    if (msg?.protocol === 'uricontrol' && msg.type === 'result' && this.pending.has(msg.id)) {
